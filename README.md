...
「房间配对 + 双向字节管道」协议默认承载在 TCP 上（中继默认 `:9009`）。relaydrop 额外把同一套协议承载到 **WebSocket over TLS** 之上，让发送方与接收方可以经过任意 TLS 终结的反向代理（如 nginx）或隧道（如 cloudflared）访问中继，而中继本身不持有证书、不占用 443。
...
```bash
# 1) 中继（本机，明文 ws/tcp 混用，监听 127.0.0.1:9009）
./target/release/relaydrop relay --listen 127.0.0.1:9009 --password SECRET

# 2) 发送方（无需指定 --code：自动生成随机密钥并打印接收命令）
./target/release/relaydrop send --relay tcp://127.0.0.1:9009 --password SECRET ./big.iso
# 终端会打印，例如：
#   On the other computer run:
#     relaydrop receive --relay tcp://127.0.0.1:9009 --code <随机> --password SECRET --out .
...
```bash
./target/release/relaydrop send --relay tcp://127.0.0.1:9009 --password SECRET ./myfolder
...
| 子命令 | 作用 |
| --- | --- |
| `relay`   | 运行中继。`--listen`（默认 `127.0.0.1:9009`）、`--password`、`--ttl`（房间超时秒） |
...
`--relay` 支持三种形式：`tcp://host:port`（裸 TCP）、`ws://host:port/path`（明文 WebSocket）、`wss://host/path`（经 TLS 终结的 WebSocket）；**省略协议头（形如 `127.0.0.1:9009`）时默认按 `tcp://` 处理**。以 `wss://` 开头即自动走加密 WebSocket。

> **破坏性变更**：本版本传输协议升级为「清单式多文件传输」（`Manifest` + `DataChunk` + `FileEnd` + `Done`），旧版单文件 `FileMeta` 客户端无法互通，发送方与接收方需同时升级。

> **破坏性变更（多源）**：`send` 现在支持一次发送多个文件/目录/混合；`Manifest` 改为**扁平 `entries` 列表并移除 `is_folder` 字段**，每个条目的 `relpath` 相对接收方 `--out` 且保留顶层名。旧版（含 `is_folder` 的）客户端无法与本版互通，发送方与接收方须同时升级。

> **环境变量**：所有命令行参数也都可通过环境变量设置——`RELAYDROP_` 前缀加参数长名大写，例如 `RELAYDROP_RELAY`、`RELAYDROP_PASSWORD`、`RELAYDROP_CODE`、`RELAYDROP_PATH`、`RELAYDROP_OUT`。优先级为 **命令行 > 环境变量 > 默认值**；完整变量表与 systemd 集成方式见 [docs/usage.md](docs/usage.md) 的「环境变量」一节。
