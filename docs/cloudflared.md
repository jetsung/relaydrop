...
- **`ws://` 前缀**：`ingress` 的 `service` 写成 `ws://127.0.0.1:9009`，cloudflared 会透明代理 WebSocket；
...
- 能连上但很快断开：检查 `ingress` 的 `service` 是否指向正确的本地端口（9009），以及 `proxy_read_timeout` 无需在此配置（cloudflared 默认长连接）。
