...
> **环境变量**：所有命令行参数也都可通过环境变量设置——`RELAYDROP_` 前缀加参数长名大写，例如 `RELAYDROP_RELAY`、`RELAYDROP_PASSWORD`、`RELAYDROP_CODE`、`RELAYDROP_PATH`、`RELAYDROP_OUT`。优先级为 **命令行 > 环境变量 > 默认值**；完整变量表见 [docs/env.md](./env.md)。

## 文档

- [docs/install.md](./install.md) — VPS 中继安装与三种部署方式
- [docs/nginx.md](./nginx.md) — nginx 反向代理完整配置（含回源 IP 白名单、防火墙）
- [docs/cloudflare.md](./cloudflare.md) — Cloudflare 侧配置
- [docs/cloudflared.md](./cloudflared.md) — cloudflared 隧道（命名隧道 + systemd 守护）
- [docs/usage.md](./usage.md) — 客户端完整用法、示例与 FAQ
- [docs/env.md](./env.md) — 环境变量完整变量表、优先级与 systemd 集成
- [docs/docker.md](./docker.md) — Docker 镜像与容器运行用法
- [docs/security.md](./security.md) — 安全提示与方案权衡
- [docs/protocol.md](./protocol.md) — 开发者向：线协议、加密信封与密钥派生
...
- 更多排错与示例见 [docs/usage.md](./usage.md)。
...
用 Docker 部署中继时的 compose `.env` 配置与容器运行示例见 [docs/docker.md](./docker.md)；环境变量完整变量表见 [docs/env.md](./env.md)（唯一事实来源）。
...
`compose.yaml` 通过 `env_file` 读取宿主机 `./.env`（`required: false`）注入 `RELAYDROP_*` 环境变量；`.env` 文件写法与 `docker run -e` 用的是同一套变量，完整变量表见 [docs/env.md](./env.md)。
...
- 其余变量（`RELAYDROP_TTL` 等）按需添加；完整变量表见 [docs/env.md](./env.md)。
